       ...
       <div className={`flex items-center justify-between relative transition-all duration-500 ${
            ...
            {leftNavigation.map((item) => (
              <Link
                ...
                className={`font-light uppercase tracking-wider transition-all duration-300 hover:scale-105 ${
                  ...
                  isActive(item.href)
                    ? `${isHomepage ? 'text-white border-b border-white/60' : 'text-black border-b border-black'}`
                    : `${isHomepage ? 'text-white/90 hover:text-white hover:border-b hover:border-white/40' : 'text-gray-600 hover:text-black'}`
                }`}
              >
                {item.name}
              </Link>
              ...
              className={`transition-all duration-300 hover:scale-110 ${
                isHomepage 
                  ? 'p-2 text-white/90 hover:text-white' 
          ...
          <div className={`hidden lg:flex items-center space-x-6 xl:space-x-8 ${
            ...
            {isAuthenticated && (
              <Link
                ...
                className={`font-light uppercase tracking-wider transition-all duration-300 hover:scale-105 ${
                  isHomepage 
                    ? 'text-lg md:text-xl lg:text-2xl text-white/90 hover:text-white' 
                    : 'text-sm md:text-base text-gray-600 hover:text-black'
                }`}
              >
                {user?.name || 'Profile'}
              </Link>
          ...
          <button
            className={`lg:hidden absolute right-0 top-1/2 transform -translate-y-1/2 p-2 transition-all duration-300 hover:scale-110 ${
              isHomepage ? 'text-white/90 hover:text-white' : 'text-gray-600 hover:text-black'
            }`}
              ...
              {allNavigation.map((item) => (
                <Link
                  ...
                  className={`block text-lg font-light uppercase tracking-wider transition-all duration-300 hover:translate-x-2 ${
                    isActive(item.href)
                      ? `${isHomepage ? 'text-white border-l-2 border-white pl-4' : 'text-black border-l-2 border-black pl-4'}`
                      : `${isHomepage ? 'text-white/90 hover:text-white hover:pl-2' : 'text-gray-600 hover:text-black hover:pl-2'}`
                  }`}
